...
DO $$ BEGIN ALTER TABLE "activity_log" ADD CONSTRAINT "activity_log_metadata_length" CHECK (length("activity_log"."metadata"::text) <= 10240); EXCEPTION WHEN duplicate_object THEN NULL; END $$;--> statement-breakpoint
DO $$ BEGIN ALTER TABLE "analytics_events" ADD CONSTRAINT "analytics_events_metadata_length" CHECK (length("analytics_events"."metadata"::text) <= 10240); EXCEPTION WHEN duplicate_object THEN NULL; END $$;--> statement-breakpoint
DO $$ BEGIN ALTER TABLE "pet_notes" ADD CONSTRAINT "pet_notes_content_length" CHECK (length("pet_notes"."content") <= 51200); EXCEPTION WHEN duplicate_object THEN NULL; END $$;--> statement-breakpoint
DO $$ BEGIN ALTER TABLE "pets" ADD CONSTRAINT "pets_medical_notes_length" CHECK (length("pets"."medical_notes") <= 10240); EXCEPTION WHEN duplicate_object THEN NULL; END $$;